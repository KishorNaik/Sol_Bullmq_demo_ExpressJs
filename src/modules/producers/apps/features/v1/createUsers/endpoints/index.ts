...
// Set Queues
const sendWelcomeUserEmailIntegrationEventQueues = setQueues('sendWelcomeUserEmailIntegrationEventQueues');
...
export class CreateUserController {
    ...
    logger.info(logConstruct("CreateUserController","createAsync",`Request:${JSON.stringify(request)}`));

    // Some Database related code....

    // Publish:Send an Welcome email Notification(Consumer)
    const jobRequest:WelcomeEmailNotificationIntegrationEventRequestDto=new WelcomeEmailNotificationIntegrationEventRequestDto();
    jobRequest.email=request.email;
    jobRequest.fullName=request.fullName;
    await publishQueuesAsync(sendWelcomeUserEmailIntegrationEventQueues, `send-welcome-user-email`,jobRequest);

    // Response
    const response=DataResponseFactory.Response(true,StatusCodes.CREATED,"Data Created Successfully");
