...
export class WelcomeEmailNotificationIntegrationEventHandler implements NotificationHandler<WelcomeEmailNotificationIntegrationEventService>{
  public async handle(notification: WelcomeEmailNotificationIntegrationEventService): Promise<void> {
    try
    {
      // Send mail Code.
      logger.info(`WelcomeEmailNotificationIntegrationEventHandler`,"handle",`Request:${JSON.stringify(notification.request)}`);
    }
    catch(ex){
      throw ex;
    }
...
//@region set Workers
const sendWelcomeUserEmailIntegrationEventWorkers = runWorkers(`sendWelcomeUserEmailIntegrationEventQueues`, async (job) => {
	logger.info(logConstruct('sendWelcomeUserEmailIntegrationEventWorkers', 'worker', `Job:${job.id}: started`));
